...
<?php
//http://onibusemponto.com/api/proximoBus/?latlng=-23.5109706,-46.5807412&sl=2&linha=2182
...
function finalizaJsonErro(){
  $json = "{\n\t".'"status" : "erro", '."\n\t".'"mensagem" : "Servidores da SPtrans estão fora do ar", '."\n\t".'"proximo_onibus" : {'."\n\t\t".'"prefixo" : 0, '."\n\t\t".'"latitude" : 0, '."\n\t\t".'"longitude" : 0, '."\n\t\t".'"rua" : "", '."\n\t\t".'"numero" : "", '."\n\t\t".'"distancia_texto" : "0", '."\n\t\t".'"distancia_valor" : 0' . "\n\t}\n}";
  echo $json;
  exit();
}
...
$endFim = replace_space($localFim);
    ...
    curl_setopt($ch, CURLOPT_CONNECTTIMEOUT , 15); //Timeout after 7 seconds
...
if($isCircular == 1){
	$itinerario = pegarItinerario($linha, "1");
}else{
...
foreach ($ordem as &$valor) {
	...
	if($iProximoOnibus >=0 && $iProximoOnibus <= $iLocalAtual){
		...
		//calculaDist($valor, $iProximoOnibus, $localAtual, $iLocalAtual, $itinerario);
		calculaDist($valor);
		$existProximo = true;
		break;
	}
...
function calculaDist($proximoBus){
